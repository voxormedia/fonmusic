  ...
  const plans = [
    {
      ...
      features: ["Доступ через браузер", "Все станции Jamendo", "Официальный сертификат", "Базовая поддержка"],
      highlight: false,
    },
    {
      name: "Стандарт",
      ...
      features: ["Приставка Android TV", "Автозапуск при включении", "Личный кабинет", "Приоритетная поддержка"],
      highlight: true,
    },
    {
      name: "Премиум",
      price: "1 250 000",
      usd: "$100",
      desc: "Для сетей заведений",
      features: ["Несколько точек", "Разные станции в каждой", "Расписание музыки", "Поддержка 24/7"],
      highlight: false,
    },
  ];
    ...
    <main style={{ fontFamily: "'Georgia', serif", background: "#080C12", color: "#E8EFF5", overflowX: "hidden", width: "100%" }}>
      ...
      <nav style={{ position: "fixed", top: 0, left: 0, right: 0, zIndex: 100, padding: `16px ${px}px`, display: "flex", alignItems: "center", justifyContent: "space-between", background: isScrolled ? "rgba(8,12,18,0.97)" : "transparent", backdropFilter: isScrolled ? "blur(20px)" : "none", borderBottom: isScrolled ? "1px solid rgba(201,168,76,0.15)" : "none", transition: "all 0.3s" }}>
        ...
        <div style={{ display: "flex", gap: 8 }}>
          {!m && (
            <a href="/demo" style={{ padding: "9px 18px", background: "transparent", border: "1px solid rgba(201,168,76,0.4)", color: "#C9A84C", borderRadius: 8, fontSize: 13, fontWeight: 600, textDecoration: "none" }}>
              Демо
            </a>
          )}
      ...
      <section style={{ minHeight: "100vh", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: `${m ? 100 : 120}px ${px}px ${m ? 60 : 80}px`, textAlign: "center", position: "relative", overflow: "hidden" }}>
        ...
        <h1 style={{ fontSize: m ? 36 : 64, fontWeight: 700, lineHeight: 1.15, marginBottom: 20, color: "#fff", maxWidth: 700 }}>
          Легальная музыка<br />
          <span style={{ color: "#C9A84C" }}>для вашего бизнеса</span>
        </h1>
        ...
        <div style={{ display: "flex", flexWrap: "wrap", justifyContent: "center", gap: m ? 12 : 32 }}>
          {[
            { val: "30 000+", label: "треков" },
            { val: "7 дней", label: "бесплатно" },
            { val: "100%", label: "легально" },
            { val: "📄", label: "Сертификат" },
          ].map(item => (
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
            {steps.map((step, i) => (
              <div key={step.num} style={{ display: "flex", gap: 16, alignItems: "center", padding: "20px", background: "#0D1B2A", border: `1px solid ${i === 0 ? "rgba(201,168,76,0.3)" : "rgba(255,255,255,0.06)"}`, borderRadius: 14 }}>
                <div style={{ width: 48, height: 48, borderRadius: "50%", background: i === 0 ? "rgba(201,168,76,0.15)" : "rgba(26,107,154,0.1)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 22, flexShrink: 0 }}>
                  {step.icon}
                </div>
                <div>
      ...
      <section id="pricing" style={{ padding: `64px ${px}px` }}>
        <div style={{ maxWidth: 640, margin: "0 auto" }}>
          <div style={{ textAlign: "center", marginBottom: 32 }}>
            ...
            <h2 style={{ fontSize: m ? 26 : 36, fontWeight: 700, color: "#fff" }}>Прозрачные цены</h2>
            <p style={{ fontSize: 13, color: "#8BA7BE", marginTop: 8 }}>По курсу ЦБ РУз на дату оплаты</p>
          </div>
          <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
            {plans.map(plan => (
              <div key={plan.name} style={{ padding: "24px", background: plan.highlight ? "linear-gradient(135deg, rgba(26,107,154,0.3), rgba(13,61,94,0.3))" : "#0D1B2A", border: `2px solid ${plan.highlight ? "#C9A84C" : "rgba(255,255,255,0.06)"}`, borderRadius: 16, position: "relative" }}>
                ...
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 16 }}>
                  ...
                  <div style={{ textAlign: "right" }}>
                    ...
                    <div style={{ fontSize: 11, color: "#8BA7BE" }}>в месяц</div>
                  </div>
                </div>
      ...
      <section style={{ padding: `64px ${px}px`, background: "#0A0F18" }}>
        <div style={{ maxWidth: 640, margin: "0 auto" }}>
          <div style={{ background: "linear-gradient(135deg, #0D1B2A, #162435)", border: "1px solid rgba(201,168,76,0.3)", borderRadius: 20, padding: m ? 28 : 40 }}>
            <div style={{ fontSize: 11, color: "#C9A84C", letterSpacing: "0.1em", marginBottom: 12 }}>FONMUSIC PLAYER</div>
            <h3 style={{ fontSize: m ? 22 : 28, fontWeight: 700, color: "#fff", marginBottom: 12 }}>
              🎵 Приставка для бизнеса
            </h3>
            <p style={{ fontSize: 14, color: "#8BA7BE", lineHeight: 1.7, marginBottom: 20 }}>
              Android TV приставка с предустановленным плеером. Включили — музыка играет. Не нужен компьютер или телефон.
            </p>
            ...
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
              <div>
                ...
                <div style={{ fontSize: 12, color: "#8BA7BE" }}>разовая оплата</div>
              </div>
        ...
        <div style={{ maxWidth: 480, margin: "0 auto", position: "relative" }}>
          <div style={{ textAlign: "center", marginBottom: 32 }}>
            ...
            <p style={{ fontSize: 14, color: "#8BA7BE", lineHeight: 1.6 }}>
              Оставьте заявку — мы подключим музыку для вашего заведения уже сегодня
            </p>
          </div>
          <LeadForm />
      ...
      <footer style={{ padding: `28px ${px}px`, borderTop: "1px solid rgba(255,255,255,0.06)" }}>
        <div style={{ maxWidth: 640, margin: "0 auto" }}>
          <div style={{ display: "flex", flexDirection: m ? "column" : "row", justifyContent: "space-between", alignItems: m ? "center" : "center", gap: 16, textAlign: "center" }}>
            <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
