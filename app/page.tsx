...
function LeadForm() {
  ...
  if (sent) return (
    <div style={{ textAlign: "center", padding: 32, background: "rgba(34,197,94,0.1)", border: "1px solid rgba(34,197,94,0.3)", borderRadius: 16 }}>
  ...
  return (
    <div style={{ width: "100%", maxWidth: 480, margin: "0 auto" }}>
      ...
      <button onClick={send} disabled={loading} style={{ width: "100%", padding: 18, background: "#C9A84C", color: "#080C12", border: "none", borderRadius: 8, fontSize: 16, fontWeight: 700, cursor: "pointer", boxSizing: "border-box", boxShadow: "0 8px 32px rgba(201,168,76,0.3)" }}>
...
export default function Home() {
  ...
  useEffect(() => {
    const t = setInterval(() => setActiveGenre(p => (p + 1) % genres.length), 2000);
  ...
  const plans = [
    ...
    { name: "Премиум", price: "1 250 000", usd: "$100", desc: "Для сетей от 5 точек", features: ["Скидка от 20%", "Личный кабинет", "Разные жанры в каждой точке", "Поддержка 24/7"], highlight: false },
  ];
  ...
  return (
    <main style={{ fontFamily: "'Georgia', serif", background: "#080C12", color: "#E8EFF5", overflowX: "hidden", width: "100%" }}>
      ...
      {/* NAV */}
      <nav style={{ position: "fixed", top: 0, left: 0, right: 0, zIndex: 100, padding: `16px ${px}px`, display: "flex", alignItems: "center", justifyContent: "space-between", background: isScrolled ? "rgba(8,12,18,0.95)" : "transparent", backdropFilter: isScrolled ? "blur(20px)" : "none", borderBottom: isScrolled ? "1px solid rgba(201,168,76,0.15)" : "none", transition: "all 0.4s ease" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
          <div style={{ width: 6, height: 24, background: "#C9A84C", borderRadius: 2 }} />
          <span style={{ fontSize: m ? 18 : 22, fontWeight: 700, color: "#fff" }}>FonMusic</span>
        </div>
        {!m && (
        ...
        <a href="#contact" style={{ padding: m ? "8px 14px" : "10px 24px", background: "#C9A84C", color: "#080C12", borderRadius: 6, fontSize: m ? 12 : 14, fontWeight: 700, textDecoration: "none", whiteSpace: "nowrap" }}>
          {m ? "Начать" : "Начать бесплатно"}
        </a>
      </nav>

      {/* HERO */}
      <section style={{ minHeight: "100vh", display: "flex", alignItems: "center", padding: `${m ? 100 : 120}px ${px}px ${m ? 60 : 80}px`, position: "relative", overflow: "hidden" }}>
        <div style={{ position: "absolute", top: "10%", right: "-5%", width: m ? 250 : 600, height: m ? 250 : 600, borderRadius: "50%", background: "radial-gradient(circle, rgba(26,107,154,0.15) 0%, transparent 70%)", pointerEvents: "none" }} />
        <div style={{ maxWidth: 1200, margin: "0 auto", width: "100%" }}>
          <div style={{ display: "grid", gridTemplateColumns: m ? "1fr" : "1fr 1fr", gap: m ? 40 : 80, alignItems: "center" }}>
            <div>
              ...
              <h1 style={{ fontSize: m ? 38 : 64, fontWeight: 400, lineHeight: 1.1, marginBottom: 20, color: "#fff", fontStyle: "italic" }}>
                Музыка для<br />
                <span style={{ color: "#C9A84C", fontStyle: "normal", fontWeight: 700 }}>вашего бизнеса</span>
              </h1>
              <p style={{ fontSize: m ? 15 : 18, color: "#8BA7BE", lineHeight: 1.7, marginBottom: 32, maxWidth: 480 }}>
                Легальная фоновая музыка для ресторанов, магазинов и кафе в Узбекистане. Официальный сертификат. Автоматически.
              </p>
              <div style={{ display: "flex", gap: 12, alignItems: "center", flexWrap: "wrap" }}>
                <a href="#contact" style={{ padding: m ? "13px 22px" : "16px 36px", background: "#C9A84C", color: "#080C12", borderRadius: 8, fontSize: m ? 14 : 16, fontWeight: 700, textDecoration: "none" }}>
                  Первые 10 дней бесплатно →
                </a>
              </div>
              <div style={{ display: "flex", gap: m ? 24 : 40, marginTop: 36 }}>
                {[["600K+", "треков"], ["27", "жанров"], ["100%", "легально"]].map(([val, label]) => (
                  <div key={label}>
                    <div style={{ fontSize: m ? 22 : 28, fontWeight: 700, color: "#C9A84C" }}>{val}</div>
            ...
            <div style={{ position: "relative" }}>
              <div style={{ background: "linear-gradient(135deg, #0D1B2A, #162435)", border: "1px solid rgba(201,168,76,0.2)", borderRadius: 20, padding: m ? 24 : 40, boxShadow: "0 40px 80px rgba(0,0,0,0.5)" }}>
                <div style={{ fontSize: 11, color: "#C9A84C", letterSpacing: "0.1em", marginBottom: 16, fontFamily: "monospace" }}>▶ СЕЙЧАС ИГРАЕТ</div>
                <div style={{ fontSize: m ? 32 : 48, fontWeight: 700, color: "#fff", marginBottom: 8, fontStyle: "italic" }}>{genres[activeGenre]}</div>
              ...
              <div style={{ position: "absolute", top: -14, right: m ? -8 : -20, background: "#C9A84C", color: "#080C12", padding: "10px 14px", borderRadius: 10, fontSize: 11, fontWeight: 700, boxShadow: "0 8px 24px rgba(201,168,76,0.4)" }}>
      ...
      {/* PROBLEM */}
      <section style={{ padding: `80px ${px}px`, background: "#0A0F18" }}>
        <div style={{ maxWidth: 1200, margin: "0 auto" }}>
          <div style={{ textAlign: "center", marginBottom: 48 }}>
            <div style={{ fontSize: 11, color: "#C9A84C", letterSpacing: "0.15em", marginBottom: 12, fontFamily: "monospace" }}>ПРОБЛЕМА</div>
            <h2 style={{ fontSize: m ? 28 : 44, fontWeight: 700, color: "#fff" }}>Незаконная музыка —<br />угроза для бизнеса</h2>
          </div>
          <div style={{ display: "grid", gridTemplateColumns: m ? "1fr" : "1fr 1fr 1fr", gap: 16 }}>
            {[
              ...
              <div key={item.title} style={{ padding: m ? 24 : 32, background: "#0D1B2A", border: "1px solid rgba(255,255,255,0.06)", borderRadius: 16 }}>
      ...
      {/* HOW IT WORKS */}
      <section id="how" style={{ padding: `80px ${px}px` }}>
        <div style={{ maxWidth: 1200, margin: "0 auto" }}>
          <div style={{ textAlign: "center", marginBottom: 48 }}>
            <div style={{ fontSize: 11, color: "#C9A84C", letterSpacing: "0.15em", marginBottom: 12, fontFamily: "monospace" }}>КАК ЭТО РАБОТАЕТ</div>
            <h2 style={{ fontSize: m ? 28 : 44, fontWeight: 700, color: "#fff" }}>Просто. Быстро. Легально.</h2>
          </div>
          <div style={{ display: "grid", gridTemplateColumns: m ? "1fr 1fr" : "1fr 1fr 1fr 1fr", gap: 12 }}>
            {steps.map((step, i) => (
              <div key={step.num} style={{ padding: m ? 20 : 32, background: "#0D1B2A", border: `1px solid ${i === 0 ? "rgba(201,168,76,0.4)" : "rgba(255,255,255,0.06)"}`, borderRadius: 16 }}>
                <div style={{ fontSize: m ? 28 : 48, fontWeight: 700, color: i === 0 ? "#C9A84C" : "rgba(26,107,154,0.5)", marginBottom: 10 }}>{step.num}</div>
                <h3 style={{ fontSize: m ? 13 : 16, fontWeight: 700, color: "#fff", marginBottom: 8 }}>{step.title}</h3>
      ...
      {/* PRICING */}
      <section id="pricing" style={{ padding: `80px ${px}px`, background: "#0A0F18" }}>
        <div style={{ maxWidth: 1200, margin: "0 auto" }}>
          <div style={{ textAlign: "center", marginBottom: 48 }}>
            <div style={{ fontSize: 11, color: "#C9A84C", letterSpacing: "0.15em", marginBottom: 12, fontFamily: "monospace" }}>ТАРИФЫ</div>
            <h2 style={{ fontSize: m ? 28 : 44, fontWeight: 700, color: "#fff" }}>Прозрачные цены</h2>
          ...
          <div style={{ display: "grid", gridTemplateColumns: m ? "1fr" : "1fr 1fr 1fr", gap: m ? 16 : 24 }}>
            {plans.map(plan => (
              <div key={plan.name} style={{ padding: m ? 28 : 40, background: plan.highlight ? "linear-gradient(135deg, #1A6B9A, #0D3D5E)" : "#0D1B2A", border: `2px solid ${plan.highlight ? "#C9A84C" : "rgba(255,255,255,0.06)"}`, borderRadius: 20, position: "relative", boxShadow: plan.highlight && !m ? "0 20px 60px rgba(26,107,154,0.3)" : "none" }}>
          ...
          {/* FONMUSIC PLAYER */}
          <div style={{ marginTop: 40, background: "linear-gradient(135deg, #0D1B2A, #162435)", border: "1px solid rgba(201,168,76,0.3)", borderRadius: 20, padding: m ? 28 : 40, display: "flex", flexDirection: m ? "column" : "row", alignItems: m ? "flex-start" : "center", gap: 28 }}>
            <div style={{ fontSize: 56 }}>🎵</div>
            <div style={{ flex: 1 }}>
              <div style={{ fontSize: 11, color: "#C9A84C", letterSpacing: "0.1em", marginBottom: 8, fontFamily: "monospace" }}>ОПЦИОНАЛЬНО</div>
              <h3 style={{ fontSize: m ? 20 : 24, fontWeight: 700, color: "#fff", marginBottom: 8 }}>FonMusic Player</h3>
            ...
            <div style={{ textAlign: m ? "left" : "center", minWidth: 130 }}>
              ...
              <a href="#contact" style={{ display: "block", padding: "12px 20px", background: "#C9A84C", color: "#080C12", borderRadius: 8, fontSize: 14, fontWeight: 700, textDecoration: "none", textAlign: "center" }}>Заказать →</a>
            </div>
          </div>
      ...
      {/* CTA */}
      <section id="contact" style={{ padding: `80px ${px}px`, position: "relative", overflow: "hidden" }}>
        ...
        <div style={{ maxWidth: 700, margin: "0 auto", textAlign: "center", position: "relative" }}>
          ...
          <h2 style={{ fontSize: m ? 36 : 56, fontWeight: 700, color: "#fff", marginBottom: 20, fontStyle: "italic" }}>
            Первые 10 дней —<br />
            <span style={{ color: "#C9A84C", fontStyle: "normal" }}>бесплатно</span>
          </h2>
          <p style={{ fontSize: m ? 15 : 18, color: "#8BA7BE", lineHeight: 1.7, marginBottom: 40 }}>
      ...
      {/* FOOTER */}
      <footer style={{ padding: `24px ${px}px`, borderTop: "1px solid rgba(255,255,255,0.06)", display: "flex", flexDirection: m ? "column" : "row", justifyContent: "space-between", alignItems: "center", gap: 12, fontSize: 12, color: "#8BA7BE", textAlign: "center" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
