...
function ScheduleEditor({ client, scheduleItems, accent, onSave, onCancel }: any) {
  ...
  const save = async () => {
    ...
    try {
      ...
      const newSlots = normalizedSlots.map((slot: any) => ({
      ...
      const updatedItems = normalizedSlots.map((slot: any) => ({
        ...slot,
  ...
  return (
    <div>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 12 }}>
        ...
        <button
          onClick={() => setSlots(normalizeScheduleSlots([...slots, {
            start_time: "12:00:00",
            end_time: "13:00:00",
            selected_station_key: "best_of_radio",
          }]))}
        ...
        {canUseSchedule && scheduleItems.length > 0 && (
          <div className="schedule-card" style={{ background: "rgba(15,45,100,0.4)", backdropFilter: "blur(20px)", border: "1px solid rgba(255,255,255,0.08)", borderRadius: 18, overflow: "hidden", marginBottom: 10 }}>
            <div style={{ width: "100%", padding: "16px 20px", background: "transparent", border: "none", fontFamily: "Georgia, serif", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
              ...
              <span style={{ color: "#22C55E", fontSize: 12, fontWeight: 700 }}>Авто</span>
            </div>

            {showTimeline && (
  <div style={{ borderTop: "1px solid rgba(255,255,255,0.06)", padding: "12px 16px" }}>
    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 10 }}>
      <div style={{ fontSize: 13, color: "#8BA7BE" }}>{showScheduleEditor ? "Редактирование" : "Сегодня"}</div>
      <button onClick={() => setShowScheduleEditor(!showScheduleEditor)} style={{ background: "rgba(255,255,255,0.06)", border: "1px solid rgba(255,255,255,0.1)", borderRadius: 8, cursor: "pointer", fontSize: 13, color: "#fff", padding: "6px 12px", fontFamily: "Georgia, serif", flexShrink: 0 }}>
  ...
  {showBox && (
    <div style={{ padding: "0 20px 24px", borderTop: "1px solid rgba(201,168,76,0.1)" }}>
      ...
      <div style={{ fontSize: 11, color: "#4a5a6a", marginBottom: 14, textAlign: "center" }}>
        750 000 сум · 500 000 сум при оплате за 3 месяца · бесплатно: Базовый 12 мес. · Стандарт 9 мес. · Премиум 6 мес.
      </div>
